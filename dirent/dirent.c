...
DIR *opendir(const char* name)
{
	...
	size_t ret = mbstowcs(wname, name, _MAX_PATH+2);
	wlen = (int)wcslen(wname)+1;
	if (ret == (size_t)-1) {
...
struct dirent* readdir(DIR* dirp)
{
	...
	size_t ret = wcstombs(dirp->ent.d_name, dirp->wfd.cFileName, _countof(dirp->ent.d_name));
	len = (int)strlen(dirp->ent.d_name)+1;
	if (ret == (size_t)-1) {
